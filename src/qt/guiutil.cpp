...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Somnio address (e.g. %1)").arg("TTh3VPSJA1zsit4sHqVVn92ARKDuM3xjTC"));
#endif
...
{
    // return if URI is not valid or is no Somnio: URI
...
{
    // Convert somnio:// to somnio:
    //
    //    Cannot handle this later, because somnio:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open somnio.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Somnio.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Somnio.lnk
...
{
    return GetAutostartDir() / "somnio.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a somnio.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Somnio\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the somnio app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add somnio app to startup item list
