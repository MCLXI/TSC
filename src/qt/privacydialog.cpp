                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zAIS ought to be enough for anybody." - Bill Gates, 2017
    ui->zAISpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAISSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzAIS->setEnabled(false);
        ui->pushButtonMintzAIS->setToolTip(tr("zAIS is currently disabled due to maintenance."));

        ui->pushButtonSpendzAIS->setEnabled(false);
        ui->pushButtonSpendzAIS->setToolTip(tr("zAIS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAISpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzAIS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAIS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAIS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAIS in ") +
...
void PrivacyDialog::on_pushButtonSpendzAIS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAIS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zAIS
        sendzAIS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAIS
    sendzAIS();
}

void PrivacyDialog::on_pushButtonZAisportControl_clicked()
{
    ZAisportControlDialog* zAisportControl = new ZAisportControlDialog(this);
    zAisportControl->setModel(walletModel);
    zAisportControl->exec();
}

void PrivacyDialog::setZAisportControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAisportSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAIS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Aisport Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAISpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAIS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " AIS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAIS</b>";
    ...
    // use mints from zAisport selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAisportControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAisportControlDialog::GetSelectedMints();
    }
    ...
    // Clear zaisport selector in case it was used
    ZAisportControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAisport Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Aisport, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAisport Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAIS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAIS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAIS "));
    ui->labelzAISAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzAISSyncStatus->setVisible(fShow);
}
