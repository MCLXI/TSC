                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTSCB ought to be enough for anybody." - Bill Gates, 2017
    ui->zTSCBpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTSCBSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTSCB->setEnabled(false);
        ui->pushButtonMintzTSCB->setToolTip(tr("zTSCB is currently disabled due to maintenance."));

        ui->pushButtonSpendzTSCB->setEnabled(false);
        ui->pushButtonSpendzTSCB->setToolTip(tr("zTSCB is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTSCBpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTSCB_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSCB is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTSCB...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTSCB in ") +
...
void PrivacyDialog::on_pushButtonSpendzTSCB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSCB is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTSCB
        sendzTSCB();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTSCB
    sendzTSCB();
}

void PrivacyDialog::on_pushButtonZTSCBControl_clicked()
{
    ZTSCBControlDialog* zTSCBControl = new ZTSCBControlDialog(this);
    zTSCBControl->setModel(walletModel);
    zTSCBControl->exec();
}

void PrivacyDialog::setZTSCBControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTSCBSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTSCB()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TSCB Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTSCBpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTSCB is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TSCB </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTSCB</b>";
    ...
    // use mints from zTSCB selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTSCBControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTSCBControlDialog::GetSelectedMints();
    }
    ...
    // Clear zTSCB selector in case it was used
    ZTSCBControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTSCB Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TSCB, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTSCB Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTSCB </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTSCB "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTSCB "));
    ui->labelzTSCBAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTSCBSyncStatus->setVisible(fShow);
}
