...
void PrivacyDialog::on_pushButtonZSomnioControl_clicked()
{
    ZSomnioControlDialog* zSomnioControl = new ZSomnioControlDialog(this);
    zSomnioControl->setModel(walletModel);
    zSomnioControl->exec();
}

void PrivacyDialog::setZSomnioControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSomnioSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSMN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Somnio Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SMN </b>will be added to the standard transaction fees!<br />";
    ...
    // use mints from zSomnio selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSomnioControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSomnioControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsomnio selector in case it was used
    ZSomnioControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSomnio Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Somnio, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSomnio Mint");
